...
export function useOnboardingWizard(workflowId?: string) {
  const { user } = useAuth()
  ...
  useEffect(() => {
    if (workflow) {
      ...
      setState({
        ...
        participantId: workflow.reference_id,
  ...
  const createWorkflow = useMutation({
    ...
    onSuccess: (data) => {
      setState((prev) => ({
        ...prev,
        workflowId: data.id,
        participantId: data.reference_id,
      }))
    },
  })
  ...
  const saveStageData = useMutation({
    mutationFn: async ({
      ...
      const wfId = overrideWorkflowId || state.workflowId
      if (!wfId) return

      const newStageData = { ...state.stageData, [stageNumber]: data }
      const newCompleted = moveToNext && !state.completedStages.includes(stageNumber)
